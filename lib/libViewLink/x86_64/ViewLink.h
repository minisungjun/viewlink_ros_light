 ...
 *          This SDK is suitable for use with F, T, TL, TIR, TIRM, TM, Z-fusionS, Z6KA7, XIR, ZIR, QIR19, Z36N, U30TIR
 *Z-fusion, Q40TIR \n We provide multiple binary libraries for different OS and uniform interfaces. ViewLink SDK is
 *currently supported on the following OS:\n
 *
 *			windows(32bit, 64bit)\n
 *
...
struct _VLK_DEV_TELEMETRY
{
  ...
  short sZoomMagTimes;                 ///< camera magnification times
 ...
 *  @param iBufLen device status data length, it must be equal to the size of struct matchs iType, e.g: iBufLen ==
 * sizeof(VLK_DEV_MODEL)
 *  @return return 0 anyway
 *  @details VLK_DevStatus_CB will be called by SDK once device new status is received, developers can easily get Gimbal
 * latest status
 ...
 *  @param pConnParam connect information, including connection type (TCP or serial port), ip address, port, serial port
 * name, baudrate
 *  @param pConnStatusCB  connnection status callback function, it will be called by SDK when device is connected or
 * disconnected
 *  @param pUserParam user parameters, will be passed back when pConnStatusCB is called
 *  @return VLK_ERROR_NO_ERROR on success, error code otherwise
 *  @details this function is asynchronous, the connect result will be passed back by callback function, do not use
 * return value to judge connect result. SDK allows only one connection, it means if you create a new connection, the
 * previous one will be disconnected automatically
 */
...
/** @brief Control Gimbal yaw and pitch (velocity mode)
...
/** @brief Stop moving
 *  @details once you call VLK_Move, Gimbal will keep moving until it reach the yaw and pitch limitation or VLK_Stop is
 * called
 */
...
/** @brief Stop zoom
 *  @details once you call VLK_ZoomIn or VLK_ZoomOut, Camera will keep zooming until it reach the limitation or
 * VLK_StopZoom is called
 */
 ...
 *  @param iVideoWidth video image width, e.g: 1920, 1280
 *  @param iVideoHeight video image height, e.g: 1080, 720
 *  @details we usually use VLK_TRACK_TEMPLATE_SIZE_AUTO for pParam->emTrackTempSize and VLK_SENSOR_VISIBLE1 for
 * pParam->emTrackSensor
 */
VLK_API void VLK_CALL VLK_TrackTargetPositionEx(const VLK_TRACK_MODE_PARAM* pParam, int iX, int iY, int iVideoWidth,
                                                int iVideoHeight);
...
/** @brief Stop focus
 *  @details  once you call VLK_FocusIn or VLK_FocusOut, Camera will keep focusing until it reach the limitation or
 * VLK_StopFocus is called
 */
...
/** @brief Turn to specific yaw and pitch (unit:deg)
